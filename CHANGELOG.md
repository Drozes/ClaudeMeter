...
- **JSON schema migration**: `UsageAPIResponse` now decodes `five_hour`, `seven_day`, `seven_day_opus`, and `seven_day_sonnet` directly. The legacy `session` / `weekly` / `per_model` fields are retained as Optional so a future Anthropic restoration would still parse. `normalizeJSONResponse` prefers the new keys and falls back to the legacy ones when present.
- **JSON circuit breaker auto-resets**: tracks a `jsonCircuitOpenedAt` timestamp on trip and re-closes after a 30-minute cooldown. Worst case: JSON gets retried every 30 minutes; persistent breakage immediately re-opens the circuit (5 fresh failures) without thrashing.
- **Skip the dead in-page click on the JSON happy path**: when the last successful fetch went via JSON and the JSON circuit is closed, `unifiedRefresh` now calls `scrapeAndDistribute` directly instead of clicking the broken in-page "Refresh" button and waiting 1.5s. The click-and-wait still runs as the fallback when DOM is the active path. Side effects: faster background updates (no 1.5s pre-scrape delay) and no more `click result: not_found` log spam.
- **Plan tier chip restored** ("MAX 20X" et al.): `sectionTitleText` was anchoring on a section title containing the word "plan", but the JSON normalizer emits "Session limit" / "Weekly limit". The check now also matches "session", so the chip renders on the first section header again. The cached slug and `planTierDisplayName` mapping were already correct.
